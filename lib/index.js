...
const { setRandomNowPlaying, getChannel, catchErrors } = require("./util.js");
...
const { ANNOUNCEMENTS_ID, PHOTO_GALLERY_ID } = require("./constants");
...
client.login(process.env.TOKEN)
  ...
  }, (reason) => { catchErrors(client, reason) })
...
var getSocialMedia = async (pgc) => {
  ...
  // cron.schedule('*/15 * * * *', () => {
  //   instagram.getLatestInstagramPost(pgc, client)
  //   // instagram.getBroadcasting(client)
  //   twitter.getLatestTwitterPost(pgc, client)
  // }, true);
...
client.on("message", message => {
  ...
  if (Math.random() <= 0.25 && message.author.id !== '294349205391147009' && message.content.slice(0, 2) !== "r!"){
    if (message.content.slice(-3) === "owo"){
      message.react(message.guild.emojis.cache.find(emoji => emoji.name === 'reolOwo').id);
    } else if (message.channel.id === (getChannel(ANNOUNCEMENTS_ID) || message.content.slice(-1) === '!')){
      message.react(message.guild.emojis.cache.find(emoji => emoji.name === 'loveReol').id);
    }
  }
