...
client.login(process.env.TOKEN)
  .then(() => {
    ...
    let tag = require('child_process')
    .execSync('git describe --abbrev=0').toString();

    let msg = `I'm updated! Commit ${revision}, ${tag}`;

    client.channels.get((process.env.NODE_ENV !== undefined) ? TEST_ID : REOL_CHAN_ID).send(msg);
