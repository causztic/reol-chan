...
client.login(process.env.TOKEN)
  .then(() => {
    setRandomNowPlaying()
    let music_chn = client.channels.get(MUSIC_VOICE_ID)
    music_chn.join().then(connection => console.log('Connected')).catch(console.error);
    if (process.env.DATABASE_URL){
...
var getLatestInstagramPost = (pgc, result) => {
...
var getInstagram = (pgc) => {
  pgc.query('SELECT post_id FROM Instagram ORDER BY ID ASC LIMIT 1;', (err, result) => {
    ...
    if (err) catchErrors(err);
    cron.schedule('*/30 * * * *', () => {
      getLatestInstagramPost(pgc, result)
      setRandomNowPlaying()
    }, true);
  });
}
