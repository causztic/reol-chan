...
// Runs the checks required to make sure that the user is able to claim a role, and assigns it if they all pass.
let claimRole = (msg, role, channel) => {
  ...
  if (!userRoles.has(role.id) && userRoles.has(memberRole.id)) {
    // Prevents the user from claiming more than one color role and one regional role
    if (!userRoles.some(r => COLOR_ROLES.includes(r.name)) || !userRoles.some(r => REGION_ROLES.includes(r.name))) {
      msg.member.addRole(role);
      channel.send(`${msg.author} now has the ${role.name} role!`);
    } else {
      msg.reply("You have already claimed one of that type of role (regional or color)! Use r!rmrole [role name] and try again.");
    }
  } else {
...
module.exports = { catchErrors, setRandomNowPlaying, getChannel, inCommandChannel, claimRole }
