      ...
      if (!memberRoles.has(role.id)) {		// Only allows this to run if the user is not already a Member
        ...
        channel.send(`Welcome to the server, ${msg.author}! <:wutGiga:297897855727697921>`);
			...
			// Prevents this generalized code from allowing random people to self-assign the admin, moderator, designer, or bae roles.
				...
				if (!memberRoles.has(role.id) && memberRoles.has(memberRole.id)) {
					// Prevents the user from claiming more than one color role and one regional role
					if (!memberRoles.some(r => COLOR_ROLES.includes(r.name)) || !memberRoles.some(r => REGION_ROLES.includes(r.name))) {
						msg.member.addRole(role);
						channel.send(`${msg.author} now has the ${role.name} role!`);
					}
				} else {
					msg.reply("Either you already have a color/region role or you have not joined the server with \'r!giverole member\'! Use r!rmrole [role name] and try again if you already have one.");
				}
		} else {
