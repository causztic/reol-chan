...
module.exports = class RemoveRoleCommand extends Command {
  ...
  async run(msg, args) {
    ...
    if (args.role === "Member" || args.role === "member") {
      msg.reply("You can't remove that role from yourself!");
      return;
    ...
    try {
      ...
      msg.reply("That role doesn't exist! Remember that role names are case-sensitive. Check which roles you have by clicking/long-pressing on your profile picture by a message or in the member list.");
      return;
    }

    if (msg.member.roles.has(memberRole.id) && msg.member.roles.has(role.id)) {
